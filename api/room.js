...
function discoverStore() {
  ...
  for (const k of Object.keys(env)) {
    ...
    // Skip read-only tokens explicitly. They share the prefix of the
    // read-write token, so a naive suffix match can pick the wrong one and
    // every write then fails at runtime rather than at boot.
    if (K.includes('READ_ONLY')) continue;
...
export default async function handler(req, res) {
  ...
  const GLOBAL_OPS = op === 'seen' || op === 'count';
  if (!GLOBAL_OPS && (!code || !/^[A-Z0-9]{4}$/.test(code))) {
  ...
  const GK = 'df:global:players';   // set of opaque install ids (no TTL)
  ...
  try {
    switch (op) {
      ...
      // A Redis set of opaque random ids. No fingerprinting, no IP, nothing
      // derived from the device: the id is generated client-side and is
      // meaningless outside this counter. SADD is idempotent, so a repeat
      // visit from the same browser does not double count.
      case 'seen': {
        if (!/^[a-z0-9]{6,24}$/.test(String(data || ''))) {
          return res.status(400).json({ error: 'bad id' });
        }
        const added = await redis(['SADD', GK, String(data)]);
        const total = await redis(['SCARD', GK]);
        return res.status(200).json({ total: total || 0, isNew: added === 1 });
      }
      case 'count': {
        const total = await redis(['SCARD', GK]);
